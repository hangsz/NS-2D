subroUTine Resi_SST( U,U_av, alf,muL,muL_av,muT,muT_av,Grad ,U_Rot)
    implicit none
    real(8),intent(in)::U(:,:),U_av(:,:),alf(:),muL(:),muL_av(:),muT(:),muT_av(:),Grad(:,:,:),U_Rot(:,:)
    ...
    if( schemeType==2)  then
        call artDissipation_SST(U,alf)
    else
        Dissi_SST = 0
    end if
    
    call visFlux_SST(U_av,muL_av,muT_av)
