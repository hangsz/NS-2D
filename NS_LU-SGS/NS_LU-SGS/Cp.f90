subroutine Cp_cal(filename)
    ...
    if( alive1 .AND. alive2 )  then
        ...
        !---------------------------
        !g_SAet the total numbers of cells on the wall
        num_b=0
        ...
        !---------------------------
        !divide the cells intoto the upper and lower array ,according_SA to the y cooridnates
        num_u=0
        num_l=0
        do i=1,num_b
        ...
        !---------------------------
        !sorT_SA the upper and lower cell according_SA to the x coorinates
        do j=1,num_u-1
            do i=num_u-1,j,-1
   ...
   !--------------------------------------------
    ...
    filename1 ="Cp/"//trim(filename)//"-Cp_u.dat"
    open(fileid,file=filename1)
    do i=1,num_u 
        Cp = ( U(5, cell_u(i) )-p_inf ) / ( 1.0/2*rou_inf*( u_inf**2 + v_inf**2 )  )
        write(fileid,*)  cell_center(1,cell_u(i) ),Cp
    end do
    
    close(fileid)
    
    filename1 ="Cp/"//trim(filename) //"-Cp_l.dat"
    open(fileid,file=filename1)
    do i=1,num_l
        Cp = ( U(5, cell_l(i) )-p_inf ) / ( 1.0/2*rou_inf*( u_inf**2 + v_inf**2 )  )
        write(fileid,*)  cell_center(1,cell_l(i) ),Cp
    end do
    close(fileid)
