subroutine LU_SGS_SA!( W, U, assigned, lower,upper)   !Lower-Upper Symmetric Successive Overrelexation scheme
    ...
    do i=1,ncells   !it doesn't mean that the sequence is identical to the cell order
        ...
        do j=1,4 
            ...
            !-------------------------------------
            !convective flux
            !Riemann problem'S_SA approximate solution, Roe scheme
            
            !calculate  the lower cells'S_SA n+1 step convective flux 
        ...
        if( moveNum == 0 ) then
            ...
            diag_WT = vol(cell_num)/dt(cell_num)  + omg/2.0*lamda_c(cell_num) + lamda_v(cell_num)  - vol(cell_num)*dQdW_SA(cell_num) 
        else
            diag = vol(cell_num)*( 1.0/dt(cell_num) + 3.0/2/dt_r ) + omg/2.0*lamda_c(cell_num) + lamda_v(cell_num)
            diag_WT = vol(cell_num)*( 1.0/dt(cell_num) + 3.0/2/dt_r ) + omg/2.0*lamda_c(cell_num) + lamda_v(cell_num) - vol(cell_num)*dQdW_SA(cell_num) 
        end if
    ...
    do i = ncells,1,-1
        ...
        do j=1,4
            ...
            !-------------------------------------------------
            !calculate the upper cells'S_SA n+1 convective flux 
    ...
    nu_SA = W_SA/U(1,:)
    where( nu_SA < 0.0 ) 
        nu_SA = minval( abs(nu_SA))
    end where
    
    where( nu_SA >( 5000.0*muL_inf/rou_inf ) )
        nu_SA = 5000.0*muL_inf/rou_inf
    end where
