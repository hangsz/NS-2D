subroUTine   visFlux_SST( U_av,muL_av,muT_av)  !central difference
    implicit none
    ...
    !------------------------------------------
     ...
     do i=1,nedges
        ...
        case(-1)
            ...
            UT_grad(:,1) = dot_product( Grad_UT(:,1,ncl),tij(:,i) ) * tij(:,i)
            UT_grad(:,2) = dot_product( Grad_UT(:,2,ncl),tij(:,i) ) * tij(:,i)
