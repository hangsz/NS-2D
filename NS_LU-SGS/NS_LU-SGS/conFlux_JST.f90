subroutine conFlux_JST      
    ...
    do i = 1,nedges
        ...
        !-------------------------------------
        ! using 2-Equation turbulent model , has to consider the turbulent kinetic energy.
        if( turModelNum ==2 )  then
            W_av(5) = W_av(5) + U_av(1,i)*U_av_SST(1,i)
        end if
        !-------------------------------------
        ...
        if ( ncr .GT. 0) then
            ...
            Fc(5,ncl)= Fc(5,ncl) + Vnt * U_av(5,i)        ! consider the convective flux caused by the  grid moving
