    ...
    !*************************************************************************
     ...
     do i=1,nedges
        ...
        case(-2:-1)
            ...
            gradU = dot_product( Grad(:,2,ncl),tij(:,i) ) *tij(:,i)         
            gradV = dot_product( Grad(:,3,ncl),tij(:,i) ) *tij(:,i) 
        ...
        !mu_av = mu_inf*(T_inf+ C)/(U_av(6,i)+C)*( U_av(6,i)/T_inf )**(3.0/2)
        mu_av = 1.45*U_av(6,i)**(3.0/2)/(U_av(6,i)+110.0) *1.0E-6
        k_av= gamma*R/(gamma-1)*mu_av/PrL       !  k=cp *mu/PrT  cp=gamma*R/gamma-1
