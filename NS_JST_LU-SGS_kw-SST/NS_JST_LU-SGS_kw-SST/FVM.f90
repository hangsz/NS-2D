...
subroutine Flow_init      !initialize the flow field
    ...
    muT = muL_inf*10.0**(-C2)
    UT(2,:) = C1*sqrt(u_inf**2 + v_inf**2)/ L
    UT(1,:) = muL_inf*10.0**(-C2)/rou_inf *UT(2,:)
    
    WT(1,:) = U(1,:) * UT(1,:)
    WT(2,:) = U(1,:) * UT(2,:)
    
    W(5,:) = W(5,:) + WT(1,:)   ! E= e + 0.5*v**2 + k
...
subroutine Solver          !the Solver
    ...
    do iter =1,itermax
        ...
        call muT_cal
        !-----------------
        call Mean_edge_Tur   ! its needed to calculate convective flux
        ...
        call kw_SST_solver       !calculate the muT
