subroutine Resi_SA( U,U_av, alf,muL,muL_av ,Grad )
    implicit none
   
    real(8),intent(in)::U(:,:),U_av(:,:),alf(:),muL(:),muL_av(:),Grad(:,:,:)
    ...
    call Con_flux_SA( U_av ) 
    
    if ( schemeType==2)  then
        call Art_dissipation_SA( U, alf )
    else
        Dissi_SA = 0.0 
    end if
    
    call Gradient_SA( U_av )
    call Vis_flux_SA( U_av,muL_av )
