module FVM              !finite volume method 
    ...
    real(8),allocatable::U(:,:) ,&    !the oringinal variables of every cell
                         W(:,:) , Q(:,:) ,Wn(:,:), Wn1(:,:)  , &  !the conservative variables of every cell &                 !the conservative variables of every cell
                         Rsi(:,:) , &                 !residual 
    ...
    do i=1,phase
        do iter = 1,itermax
            ...
            do count =1,iter_inner
                ...
                call Art_dissipation  !call Gradient    
                
                !calculate the gradient of every cell using the Mean_edge subroutine
                call Gradient
