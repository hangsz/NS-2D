subroutine Mean_edge  
    ...
    do i=1,nedges
        ...
        case (-1)                           !-1 represents that the edge is the aerofoil  surface         
            U_av(1,i) = U(1,ncl)
            U_av(2,i) = 0.5*( U_Rot(1,iedge(1,i)) + U_Rot(1,iedge(2,i)) )                !no slipping conditions
        ...
        case (-2)                           !-2 represents that the edge is the farfield  boundaries
            ...
            ! Vn = U(2,ncl)*vector(1,i) + U(3,ncl)*vector(2,i)  ! Temporarily, the edge state is asummed to be equal to the ncl cell state
            rou0 = U(1,ncl)
