subroutine reorder  
    ...
    if( alive1 .AND. alive2 .AND. isNewGrid=="n") then
        open(newunit=idReo,file = "LU-SGS/Assigned.dat")
        ...
        open(newunit=idReo,file = "LU-SGS/Lower_Upper.dat") 
    
            do i =1,ncells
                read(idReo,*) lower(1,i),lower(2,i),lower(3,i)
            end do
            do i =1,ncells
                read(idReo,*) upper(1,i),upper(2,i),upper(3,i)
            end do
    ...
    do i=1,nedges
        ...
        if( ncr.GT. 0) then                        ! only when the right cell exists is it necessary to continue
            do j=1,3                               ! three means that the cell is triangle, so it only has three edges
    ...
    do   
        ...
        do i=1,member_last                 ! the adjacent cell of the cell reordered previously are those to be reordered.
            ...
            do j=1,3  
                ...
                do k=1,3                   ! all the neighbers of the cell being to reordered should be tested
    ...
    do i=1,nedges
        ...
        if( plane1 .LT. plane ) then                  ! the right cell in the left cell's upper array
            do k=1,3                                  ! the ncl is stored  where the value is zero
    ...
    open(newunit=idReo,file = "LU-SGS/Lower_Upper.dat")
        ...
        do i =1,ncells
            write(idReo,*) lower(1,i),lower(2,i),lower(3,i)
        end do
        do i =1,ncells
            write(idReo,*) upper(1,i),upper(2,i),upper(3,i)
        end do
