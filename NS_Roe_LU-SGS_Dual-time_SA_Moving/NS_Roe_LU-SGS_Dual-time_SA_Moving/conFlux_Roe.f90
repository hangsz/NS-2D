subroutine conFlux   
    !------------------------------------
        ! purpose: calculate the convective flux using Roe scheme
    !------------------------------------
    implicit none
    integer::i,j
    integer::ncl,ncr
    ...
    real(8)::UL(5),UR(5)                     ! rou, u, v, w, p of the left and right state of each edge
   
    
    real(8)::ux,uy,Vn,Vn_grid                
    real(8)::d2L(5),d2R(5)
    ...
    do i=1,nedges        !traverse along the edge
        ...
        if ( ncr .GT. 0)  then  !  right cell exist
    ...
    psi = 1.0   ! limited factor--psi is no more than 1
    ...
    do i=1,nedges
        ...
        case(-2 : -1 )  
            ! on the boundaries, each edge only has left cell, so the convective flux is calculated directively.
        ...
        case default           
            ! for edges in the interal domain, convective effect has to be considered.
            do j=1,5
            ...
            Vn_grid =  0.5 * ( U_Rot(1,iedge(1,i)) + U_Rot(1,iedge(2,i)) ) * nx   +   0.5 * ( U_Rot(2,iedge(1,i)) + U_Rot(2,iedge(2,i)) ) * ny
            
            Vn_Roe = u_Roe * nx + v_Roe * ny -  Vn_grid
    
            VnL = UL(2)*nx + UL(3)*ny - Vn_grid 
            VnR = UR(2)*nx + UR(3)*ny - Vn_grid
            !-----------------------------------------
