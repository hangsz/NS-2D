subroutine LU_SGS   
    ...
    do i=1,ncells   
        ...
        ! get the cell number to be calculated.
        cell_num = assigned(i)  
        ...
        do j=1,3 
            ...
            if( edge .EQ. 0)  then       ! zero means that it has no lower edges.
                exit
            end if
                
            ! determine the cell number lower than the current cell.
            ! the lower can't be itself.
           
            if( iedge(3,edge) .NE. cell_num ) then
                cell_lower = iedge(3,edge)             ! if the left cell is its lower cell.
                sign =  -1.0                           ! the sign is negative, because positive direction is defined as left to right.
            else
            ...
            !calculate  the lower cells's n+1 step convective flux using the cell center's flow properties.
            ...
            ! the flow speed in the cell center has to subtract the grid rotating speed
            ux =  U_temp(2) - 1.0/3 * ( U_Rot(1,icell(1,cell_lower)) + U_Rot(1,icell(2,cell_lower)) + U_Rot(1,icell(3,cell_lower)) )
            uy =  U_temp(3) - 1.0/3 * ( U_Rot(2,icell(1,cell_lower)) + U_Rot(2,icell(2,cell_lower)) + U_Rot(2,icell(3,cell_lower)) )
            ...
            ux =  U(2,cell_lower) - 1.0/3 * ( U_Rot(1,icell(1,cell_lower)) + U_Rot(1,icell(2,cell_lower)) + U_Rot(1,icell(3,cell_lower)) )
            uy =  U(3,cell_lower) - 1.0/3 * ( U_Rot(2,icell(1,cell_lower)) + U_Rot(2,icell(2,cell_lower)) + U_Rot(2,icell(3,cell_lower)) )
            Vn = ux * vector(1,edge)   + uy * vector(2,edge)
            ! Vn = dot_product( U(2:3,cell_lower),vector(:,edge) )
            
            c = sqrt( gamma*U(5,cell_lower) / U(1,cell_lower) )
            
            rA_v = ds(edge) /sqrt( ( cell_center(1,cell_num)-cell_center(1,cell_lower) )**2 + ( cell_center(2,cell_num)-cell_center(2,cell_lower) )**2 )  &
                 & * max( 4.0/3/U(1,cell_lower) ,gamma/U(1,cell_lower) )*( muL(cell_lower)/PrL + muT(cell_lower)/PrT ) 
    ...
    do i = ncells,1,-1
        ...
        do j=1,3 
            ...
            ux =  U_temp(2) - 1.0/3 * ( U_Rot(1,icell(1,cell_upper)) + U_Rot(1,icell(2,cell_upper)) + U_Rot(1,icell(3,cell_upper)) )
            uy =  U_temp(3) - 1.0/3 * ( U_Rot(2,icell(1,cell_upper)) + U_Rot(2,icell(2,cell_upper)) + U_Rot(2,icell(3,cell_upper)) )        
            Vn = ux * vector(1,edge)   + uy * vector(2,edge) 
            ! Vn = dot_product( U_temp(2:3),vector(:,edge) )
            ...
            ux =  U(2,cell_upper) - 1.0/3 * ( U_Rot(1,icell(1,cell_upper)) + U_Rot(1,icell(2,cell_upper)) + U_Rot(1,icell(3,cell_upper)) )
            uy =  U(3,cell_upper) - 1.0/3 * ( U_Rot(2,icell(1,cell_upper)) + U_Rot(2,icell(2,cell_upper)) + U_Rot(2,icell(3,cell_upper)) )        
            Vn = ux * vector(1,edge)   + uy * vector(2,edge) 
    ...
    W = W + dwn 
    WT = WT + dWTn 
    !calculate the primative variables
    ...
    ! nuT is always positive. when it gets negative, set it a small postive number.
    where( nuT .LT. 0.0 ) 
        nuT = minval( abs(nuT))
    end where
