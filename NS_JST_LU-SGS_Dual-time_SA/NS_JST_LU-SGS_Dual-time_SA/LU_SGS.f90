subroutine LU_SGS   !Lower-Upper Symmetric Successive Overrelexation scheme
    ...
    do i=1,ncells   !it doesn't mean that the sequence is identical to the cell order
        ...
        diag_WT = vol(cell_num)*( 1.0/dt(cell_num) + 3.0/2/dt_r ) + omg/2.0*lamda_c(cell_num) + lamda_v(cell_num)  - vol(cell_num)*dQTdWT(cell_num) 
...
0        diag = vol(cell_num)*( 1.0/dt(cell_num) + 3.0/2/dt_r ) + omg/2.0*lamda_c(cell_num) + lamda_v(cell_num)
