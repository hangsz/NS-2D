subroutine   visFlux_SST(muL_av,muT_av)  !central difference
    implicit none
    real(8),intent(in)::muL_av(:),muT_av(:)
    ...
    !------------------------------------------
     ...
     do i=1,nedges
        ...
        case(-1)
            ...
            UT_grad(:,1) = dot_product( Grad_SST(:,1,ncl),vector(:,i) ) * vector(:,i)/ds(i)**2
            UT_grad(:,2) = dot_product( Grad_SST(:,2,ncl),vector(:,i) ) * vector(:,i)/ds(i)**2
        ...
        case default
            ...
            UT_grad = 1.0/2*( Grad_SST(:,:,ncl) + Grad_SST(:,:,ncr) )
