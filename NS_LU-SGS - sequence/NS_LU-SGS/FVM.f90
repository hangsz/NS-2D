...
subroutine Converge(flag,oldRsi)          !verify wheather the flow converge
    implicit none
    integer::i,j
    integer::flag                  !flag, 1:converge;0:disconverge
   
    real(8),intent(in)::oldRsi(:)
    real(8)::newRsi(5) 
    ...
    if( moveNum==0)  then
        if (newRsi(1)/oldRsi(1) < eps) flag = 1  
    else
        if (newRsi(1)/oldRsi(1) < eps_in) flag = 1
    end if
    
    write(*,*)  muT(100)
    write(*,*)  U(1,100),U(5,100)
    write(*,*)  U(2:3,100)
    write(*,*)  newRsi(1),newRsi(5)
    write(*,*)  newRsi(2:3)
    write(*,*)  
