subroutine LU_SGS   !Lower-Upper Symmetric Successive Overrelexation scheme
    ...
    do i=1,ncells   !it doesn't mean that the sequence is identical to the cell order
        ...
        do j=1,3 
            ...
            mu = 1.45*( U(5,cell_lower)/R/U(1,cell_lower) )**(3.0/2)/( U(5,cell_lower)/R/U(1,cell_lower)+110.0) *1.0E-6 
            rA_v = ds(edge) /sqrt( ( cell_center(1,cell_num)-cell_center(1,cell_lower) )**2 + ( cell_center(2,cell_num)-cell_center(2,cell_lower) )**2 )* max( 4.0/3/U(1,cell_lower) ,gamma/U(1,cell_lower) )*( mu/PrL ) 
    ...
    do i = ncells,1,-1
        ...
        do j=1,3 
            ...
            mu = 1.45*( U(5,cell_upper)/R/U(1,cell_upper) )**(3.0/2)/( U(5,cell_upper)/R/U(1,cell_upper)+110.0) *1.0E-6 
            rA_v = ds(edge) / sqrt( ( cell_center(1,cell_num)-cell_center(1,cell_upper) )**2  + ( cell_center(2,cell_num)-cell_center(2,cell_upper) )**2 )*max( 4.0/3/U(1,cell_upper) ,gamma/U(1,cell_upper) )*( mu/PrL ) 
            rA =omg*(  abs(Vn) + c*ds(edge) ) +rA_v
