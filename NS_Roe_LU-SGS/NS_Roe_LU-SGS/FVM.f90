module FVM              !finite volume method 
    ...
    real(8),allocatable::W(:,:)                !the conservative variables of every cell
    real(8),allocatable::U_av(:,:)
    real(8),allocatable::Grad(:,:,:)  
    real(8),allocatable::lamda_c(:) ,lamda_v(:) 
    real(8),allocatable::Fc(:,:)                  !convective flux of every cell
    real(8),allocatable::Fv(:,:)   
    real(8),allocatable::Rsi(:,:)                 !residual   
    
    real(8),allocatable::dt(:)         !time step of every cell 
...
subroutine Solver          !the Solver
    ...
    do iter = 1,itermax
        ...
        call Time_step       !calculate the time step
