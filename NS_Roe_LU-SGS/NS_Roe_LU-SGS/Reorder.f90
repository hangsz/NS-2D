subroutine Reorder  
    ...
    inquire(file = "LU-SGS/Assigned.dat",exist = alive1)
    inquire(file = "LU-SGS/Lower_upper.dat",exist = alive2)
    
    if( alive1 .AND. alive2 .AND. isNewGrid=="n") then
        open(fileid,file = "LU-SGS/Assigned.dat")
        ...
        open(fileid,file = "LU-SGS/Lower_Upper.dat") 
    
            do i =1,ncells
                read(fileid,*) lower(1,i),lower(2,i),lower(3,i)
            end do
            do i =1,ncells
                read(fileid,*) upper(1,i),upper(2,i),upper(3,i)
            end do
    ...
    adjacent = 0    !here it's set to be zero which can be useful to successive operation
    do i=1,nedges
        ...
        if(ncr.GT. 0) then 
            do j=1,3
                if( adjacent(j,ncl) .EQ. 0 ) then  !find the first number that equals zero,the cell number store here
                     adjacent(j,ncl) = ncr
                     exit                          !if stored succesfully, then it's necessary to exit the loop
                end if
            end do
    ...
    plane_cell(ncl) = 1   !ncl cell's member is 1 and plane also is 1
        ...
        do i=1,member_last
           ...
           ! write(*,*) "The cell assigned last time:",cell_last ,i, member_last
            ...
            do j=1,3 
                ...
                flag = 0
                do k=1,3  !all the neighbers of the cell being to assigned should be tested
    ...
    do i=1,nedges
       ...
       ! if(plane1 .NE. plane)  write(*,*)  i, "ncl:", ncl,plane1, "ncr:", ncr,plane
        ...
        if(plane1 .LT. plane) then                  !the right cell in the left cell's upper array
            do k=1,3                                !the ncl is stored in the first station where the value is zero
    ...
    open(fileid,file = "LU-SGS/Lower_Upper.dat")
        ...
        do i =1,ncells
            write(fileid,*) lower(1,i),lower(2,i),lower(3,i)
        end do
        do i =1,ncells
            write(fileid,*) upper(1,i),upper(2,i),upper(3,i)
        end do
