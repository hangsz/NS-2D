subroutine Con_flux   !convective flux adopte Roe scheme
    implicit none
    integer::i,j
    integer::ncl,ncr
    ...
    do i=1,nedges
        ...
        if ( ncr .GT. 0)  then 
          !the maximun value is among the last Umax and the current left and right valus
          !the minimun value is among the last Uin and the current left and right valus
            do j=1,5
    ...
    do i=1,nedges
        ...
        !        
        !        if ( d2L(j) .GT. 0.0 ) then
        !            psiL(j) = 1.0/d2L(j)* ( (d1maxL(j)**2 + epsi2L)*d2L(j) + 2.0 * d2L(j)**2*d1maxL(j) ) / ( d1maxL(j)**2 + 2*d2L(j)**2 + d1maxL(j)*d2L(j) + epsi2L )
        !        else if( d2L(j) .LT. 0.0 ) then
        !            psiL(j) = 1.0/d2L(j)* ( (d1minL(j)**2 + epsi2L)*d2L(j) + 2.0 * d2L(j)**2*d1minL(j) ) / ( d1minL(j)**2 + 2*d2L(j)**2 + d1minL(j)*d2L(j) + epsi2L )   
        !        else
        !            psiL(j) = 1.0
        !        end if
        !       
        !        psi(j,ncl)  = min( psi(j,ncl),psiL(j) )
        !        !if( psi(j,ncl) < 0.0  .OR. psi(j,ncl) >1.0 ) then
        !        !    stop "3"
        !        !else 
        !        !    write(*,*)  psi(j,ncl)
    ...
    do i=1,nedges
        ...
        case( -1 ) 
            ...
            Fc(2,ncl) = Fc(2,ncl) + U_av(5,i)*vector(1,i)
            Fc(3,ncl) = Fc(3,ncl) + U_av(5,i)*vector(2,i)
